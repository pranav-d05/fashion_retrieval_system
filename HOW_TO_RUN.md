...
2. Run the indexer:
   ```bash
   uv run python scripts/build_index.py --image-dir data/images
   ```
...
```bash
uv run python scripts/search_cli.py --open-images
```

**Single Query Mode**
If you want to run one search command directly from the terminal:
```bash
uv run python scripts/search_cli.py --query "a model in a red dress"
```
To automatically open the result images from a single query:
```bash
uv run python scripts/search_cli.py --query "a model in a red dress" --open-images
```
...
```bash
uv run python scripts/evaluate.py golden-set --dataset configs/golden_dataset.json
```
...
```bash
uv run python scripts/evaluate.py self-retrieval --num-samples 100
```
...
```bash
uv run python scripts/evaluate.py inspect-queries --query-file configs/evaluation_queries.json --copy-images
```
